...
class MuxRequest(BaseModel):
    video_url: str
    audio_url: str = None  # Dibuat opsional agar tidak error 422 jika kosong
    resolution: str
    title: str
...
async def mux_video(request: MuxRequest, background_tasks: BackgroundTasks):
    # Validasi input untuk menghindari 422
    ...
    try:
        ...
        # Jika butuh Mux (ada audio terpisah)
        if request.audio_url and request.audio_url != "None":
            cmd.extend(["-i", request.video_url, "-user_agent", ua, "-i", request.audio_url])
            cmd.extend(["-c:v", "copy", "-c:a", "aac", "-map", "0:v:0", "-map", "1:a:0"])
        else:
            # Jika video sudah ada audio, langsung copy saja
            cmd.extend(["-i", request.video_url, "-c", "copy"])
        ...
        raise HTTPException(status_code=500, detail=f"Mux gagal: {str(e)}")
